...
module.exports = function (passport) {
	...
	passport.use('cria_usuario_login', new LocalStrategy({
		usernameField: 'email',
		passwordField: 'senha',
		...
		usuarioRepository.findByEmail(email).then((usuario) => {
			if (usuario) {
				...
				usuarioRepository.create(req.body).then((novoUsuario) => {
					if (!novoUsuario) return done(null, false);
					if (novoUsuario) return done(null, novoUsuario);
				}).catch((err) => {
					return done(null, false, 'Ops...Algo de errado com o seu login'+ err.message);
				});
			}
		});
	}
	...
	//LOCAL SIGNIN
	passport.use('valida_login', new LocalStrategy({
		usernameField: 'email',
		passwordField: 'senha',
		passReqToCallback: true
		...
		usuarioRepository.findByEmail(email).then((usuario) => {
			...
			if (bcrypt.compareSync( senha,usuario.senha)) {
				...
				const response = {token, tenantId: usuario.tenantId, usuario: usuario.nome}
